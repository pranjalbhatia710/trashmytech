...
export default function NeuralBackground({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const chainDist = 90 + k * 40; // shorter connection range
      ...
      const tierPaths: Array<{ alpha: number; lineWidth: number; segs: Array<[number, number, number, number]> }> = [
        { alpha: (0.015 + k * 0.02), lineWidth: 0.15 + k * 0.05, segs: [] },
        { alpha: (0.03 + k * 0.04), lineWidth: 0.2 + k * 0.08, segs: [] },
      ];
      ...
      // Cross-chain — very subtle, sparser
      const crossDist = 35 + k * 15;
      const crossDistSq = crossDist * crossDist;
      ctx.strokeStyle = `rgba(${cr}, ${cg}, ${cb}, ${0.01 + k * 0.02})`;
      ctx.lineWidth = 0.08 + k * 0.04;
      ctx.beginPath();
      ...
      // ── Particle dots — subtle, no harsh dots ─────────────
      ctx.fillStyle = `rgb(${cr}, ${cg}, ${cb})`;
      for (const p of particles) {
        const { sx, sy, s } = proj(p.x, p.y, p.z);
        const a = pAlpha(p, k);
        if (a < 0.03) continue;
        const r = Math.max(0.2, p.size * s * (0.4 + k * 0.3));
        ctx.globalAlpha = a * 0.6; // dimmer dots
        ctx.beginPath();
